...
const Orders = (props) => {
    ...
    function makeSettleForTakeOut(id){
        firebase.firestore()
        .collection("owners").doc(shopID).collection("orders")
        .where("orderID","==",id).where("settled","==",false)
        .get().then((querySnapshot)=>{
            querySnapshot.forEach((doc)=>{
                settleID(doc.id);
            })
        ...
        <div className="order_inner">
            ...
            <h2>オーダーの検索</h2>
                    ...
                    <div className="basket">
                        ...
                        {settle ?
                        <Confirm message="精算が完了しましたか？"　action="はい" order={()=>{makeSettleForTakeOut(bill.orderID);}} closePopup={()=>{setSettle(!settle)}} />
                        :
                        null
                        }
            ...
            <table id="orderTable">
                ...
                {orders.map((item)=>(
                    ...
                    {eatIn ? <td key={"table"+item.table}>{item.table}</td>:<td key={"order"+item.orderID}>{item.orderID}</td>}<td>{item.list.map((item)=>(<a key={item}>{item}<br/></a>))}</td><td><button className="btn-flat-border" onClick={()=>{setServed(item.orderID);}}>{eatIn ? "提供完了":"調理完了"}</button></td><td><i onClick={()=>{setDelConf(item.orderID)}} className="fas fa-trash-alt"></i></td>
            ...
            {served ?
            <Confirm message="操作を完了しますか？" design="logout_conf" order={()=>makeItServerd2(served)} action="はい" closePopup={()=>{setServed(null)}} />
            :
            null
            }
